    ...
    <>
      <section className="page-header">
        ...
        <div className="page-header-row">
          <div>
            <h1 className="console-title">Transaction ledger</h1>
          </div>
          <Link href="/penny-log/new" className="console-button-primary">New Transaction</Link>
        </div>
      </section>
      ...
      <form method="GET" action="/penny-log" className="console-toolbar list-filter-bar mt-6">
        <div className="flex flex-wrap items-center justify-end gap-4">
          {hasFilters && (
            <Link href="/penny-log" className="console-button-ghost !min-h-0 !px-0 text-sm font-semibold">Reset filters</Link>
          )}
        </div>

        <div className="list-filter-grid lg:grid-cols-3">
          <div>
            <label className="console-label" htmlFor="log-status">Status</label>
            <select id="log-status" name="status" className="console-select" defaultValue={params.status || ''}>
              <option value="">All statuses</option>
              {['pending', 'success', 'failed', 'timeout', 'returned'].map((s) => (
                <option key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>
              ))}
            </select>
          </div>
          <div>
            <label className="console-label" htmlFor="log-direction">Direction</label>
          ...
          <div>
            <label className="console-label" htmlFor="log-partner">Partner</label>
            <input type="text" id="log-partner" name="partner_name" defaultValue={params.partner_name || ''} placeholder="Partner name" className="console-input" />
          </div>
          <div>
            <label className="console-label" htmlFor="log-from">From</label>
            <input type="date" id="log-from" name="date_from" defaultValue={params.date_from || ''} className="console-input" />
          </div>
          <div>
            <label className="console-label" htmlFor="log-to">To</label>
            <input type="date" id="log-to" name="date_to" defaultValue={params.date_to || ''} className="console-input" />
          </div>
          <div>
            <label className="console-label" htmlFor="log-search">Search</label>
            <input type="text" id="log-search" name="search" defaultValue={params.search || ''} placeholder="Reference, notes, partner" className="console-input" />
          </div>
        </div>

        <div className="list-filter-actions">
          <button type="submit" className="console-button-secondary">Apply Filters</button>
          <Link href="/penny-log" className="console-button-ghost">Clear</Link>
        </div>
      </form>
            ...
            {result.data.map((log) => (
              <article key={log.id} className="record-card">
                <div className="record-card-header">
                  <div>
                    ...
                    <p className="record-card-copy">
                      {log.direction} / <span className="font-mono" dir="auto">{log.reference_id || 'no reference'}</span>
                    </p>
                  </div>
                ...
                <div className="record-actions">
                  <Link href={`/penny-log/${log.id}`} className="table-action-link">View</Link>
                  <Link href={`/penny-log/${log.id}/edit`} className="table-action-link">Edit</Link>
                  <form action={deleteLog}>
                    <input type="hidden" name="id" value={log.id} />
                    <button type="submit" className="table-action-link danger">Delete</button>
                  </form>
                </div>
              </article>
            ))}
