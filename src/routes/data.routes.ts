...
export default function dataRoutes(db: Knex): Router {
  ...
  router.get('/', (_req: Request, res: Response) => {
    res.render('data/index', { title: 'Export / Import', importResult: null });
  });
  ...
  router.post('/import', importUpload.single('file'), async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!req.file) {
      ...
      res.render('data/index', { title: 'Export / Import', importResult: summary });
    } catch (err) {
