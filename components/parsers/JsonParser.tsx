...
export default function JsonParser() {
  ...
  const handleVisualize = useCallback(() => {
    ...
    try {
      ...
      alert('Cannot visualize: invalid JSON');
    }
  }, [result]);
              ...
              <form onSubmit={handleParse}>
                <div className="code-editor">
                  <div className="code-editor-toolbar">
                    <label htmlFor="json-input" className="console-inline-label">JSON input</label>
              ...
              <div className="console-panel overflow-hidden">
                <div className="px-5 py-4 border-b border-border">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-success">Valid JSON</span>
                    {result.repaired && (
                ...
                {result.stats && (
                  <div className="px-5 py-3 border-b border-border bg-page/50">
                    <div className="console-stats-grid">
                      <div className="console-stats-row"><span>Keys</span><strong>{result.stats.keys}</strong></div>
                      <div className="console-stats-row"><span>Depth</span><strong>{result.stats.depth}</strong></div>
                      <div className="console-stats-row"><span>Objects</span><strong>{result.stats.objectCount}</strong></div>
                      <div className="console-stats-row"><span>Arrays</span><strong>{result.stats.arrayCount}</strong></div>
                      <div className="console-stats-row"><span>Strings</span><strong>{result.stats.stringCount}</strong></div>
                      <div className="console-stats-row"><span>Numbers</span><strong>{result.stats.numberCount}</strong></div>
                ...
                <div className="px-5 py-3 border-b border-border">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="console-inline-label">Formatted output</span>
                    <button
                      type="button"
                      ref={formattedBtnRef}
                      onClick={() => copyToClipboard(result.formatted ?? '', formattedBtnRef.current)}
                ...
                <div className="px-5 py-3">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="console-inline-label">Minified</span>
                    <button
                      type="button"
                      ref={minifiedBtnRef}
                      onClick={() => copyToClipboard(result.minified ?? '', minifiedBtnRef.current)}
                      className="console-text-action"
                    >
                      Copy
                    </button>
              ...
              <div className="console-panel overflow-hidden border-l-4 border-l-danger">
                <div className="px-5 py-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm font-semibold text-danger">Invalid JSON</span>
                  </div>
