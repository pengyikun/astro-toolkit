        ...
        <div className="console-panel">
          <div className="console-panel-body">
            <form onSubmit={handleSubmit} className="space-y-4">
              <label htmlFor="iban-input" className="console-label">Enter IBAN</label>
            ...
            <div className="helper-list mt-5">
              <div className="helper-row">
                ...
                <span>Spaces and casing are ignored. The checker supports SEPA, UK, and broader cross-border formats.</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="console-panel overflow-hidden border-l-4 border-l-success">
                <div className="px-6 pt-6 pb-4">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-sm font-semibold text-success">Valid IBAN</span>
                  </div>
                ...
                <div className="px-6 pb-6">
                  <dl className="grid grid-cols-1 gap-x-8 divide-y divide-border sm:grid-cols-2 sm:divide-y-0">
                    <div className="space-y-0 divide-y divide-border">
                      ...
                      <div className="flex justify-between py-3">
                        <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">Check Digits</dt>
                        <dd className="text-sm text-ink font-mono">{result.check_digits}</dd>
                      </div>
                      <div className="flex justify-between py-3">
                        <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">BBAN</dt>
                    ...
                    <div className="space-y-0 divide-y divide-border">
                      {result.bank_identifier && (
                        <div className="flex justify-between py-3">
                          <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">Bank Identifier</dt>
                          <dd className="text-sm text-ink font-mono">{result.bank_identifier}</dd>
                        </div>
                      )}
                      {result.branch_identifier && (
                        <div className="flex justify-between py-3">
                          <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">Branch Identifier</dt>
                          <dd className="text-sm text-ink font-mono">{result.branch_identifier}</dd>
                        </div>
                      )}
                      {result.account_number && (
                        <div className="flex justify-between py-3">
                          <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">Account Number</dt>
                ...
                <div className="console-panel overflow-hidden mt-4">
                  <div className="px-6 py-4">
                    <div className="flex items-center gap-2 text-ink-muted">
                      ...
                      <span className="text-sm">No LEI record found for bank identifier <span className="font-mono font-medium">{result.bank_identifier}</span></span>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="console-panel overflow-hidden border-l-4 border-l-danger">
              <div className="px-6 py-6">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-semibold text-danger">Invalid IBAN</span>
                </div>
                {data?.input && (
