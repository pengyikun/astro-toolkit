...
function BoolPill({ value, trueClass, falseClass }: { value: boolean; trueClass: string; falseClass: string }) {
  return (
        ...
        <div className="console-panel">
          <div className="console-panel-body">
            <form onSubmit={handleSubmit} className="space-y-4">
              <label htmlFor="bic-input" className="console-label">Enter BIC or SWIFT code</label>
              <div className="flex flex-col gap-3 sm:flex-row">
                <input
                  ...
                  placeholder="e.g. NWBKGB2L or NWBKGB2LXXX"
                  autoComplete="off"
            ...
            <div className="helper-list mt-5">
              <div className="helper-row">
                ...
                <span>Supports both 8-character institution codes and 11-character branch-specific codes. Case does not matter.</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="console-panel overflow-hidden border-l-4 border-l-success">
                <div className="px-6 pt-6 pb-4">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-sm font-semibold text-success">Valid BIC or SWIFT</span>
                  </div>
                  ...
                  <dl className="grid grid-cols-1 gap-x-8 sm:grid-cols-2">
                    <div className="space-y-0 divide-y divide-border">
                      <div className="flex justify-between py-3">
                        <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">Institution Code</dt>
                        <dd className="text-sm text-ink font-mono">{result.institution_code}</dd>
                      </div>
                      <div className="flex justify-between py-3">
                        <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">Country</dt>
                        <dd className="text-sm text-ink">{result.country_name} ({result.country_code})</dd>
                      </div>
                      <div className="flex justify-between py-3">
                        <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">Location Code</dt>
                        <dd className="text-sm text-ink font-mono">{result.location_code}</dd>
                      </div>
                      <div className="flex justify-between py-3">
                        <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">Branch Code</dt>
                        <dd className="text-sm text-ink font-mono">{result.branch_code || 'N/A (8-char BIC)'}</dd>
                      </div>
                    </div>
                    <div className="space-y-0 divide-y divide-border">
                      <div className="flex justify-between items-center py-3">
                        <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">Primary Office</dt>
                        <dd><BoolPill value={!!result.is_primary_office} trueClass="bg-success-light text-success" falseClass="bg-page text-ink-secondary" /></dd>
                      </div>
                      <div className="flex justify-between items-center py-3">
                        <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">Test BIC</dt>
                        <dd><BoolPill value={!!result.is_test_bic} trueClass="bg-warning-light text-warning" falseClass="bg-page text-ink-secondary" /></dd>
                      </div>
                      <div className="flex justify-between items-center py-3">
                        <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">Passive Participant</dt>
                        <dd><BoolPill value={!!result.is_passive_participant} trueClass="bg-warning-light text-warning" falseClass="bg-page text-ink-secondary" /></dd>
                      </div>
                      <div className="flex justify-between items-center py-3">
                        <dt className="text-[12px] font-medium text-ink-muted uppercase tracking-wider">Reverse Billing</dt>
                        <dd><BoolPill value={!!result.is_reverse_billing} trueClass="bg-brand-light text-brand" falseClass="bg-page text-ink-secondary" /></dd>
                      </div>
                    </div>
                  </dl>
                  {leiEntity && (
            ...
            <div className="console-panel overflow-hidden border-l-4 border-l-danger">
              <div className="px-6 py-6">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-semibold text-danger">Invalid BIC or SWIFT</span>
                </div>
                {data?.input && (
